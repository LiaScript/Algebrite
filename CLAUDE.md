...
`defint` ([sources/defint.ts](sources/defint.ts)) uses `F(b)-F(a)` only where the antiderivative `F` is continuous. With numeric bounds it first looks for poles of the integrand inside the interval (`checkNoInteriorPole`: zeros of denominators, exactly for polynomials and `sin`/`cos`/`tan` of a linear argument, by a sign and minimum scan for anything else such as `1+2*cos(x)`; every candidate is confirmed by the growth of the integrand beside it) and stops there. Then `interiorJumps` adds `F(r-)-F(r+)` as one-sided limits at the singular points of `F` itself, e.g. of `tan(x/2)` in `arctan(tan(x/2)/sqrt(3))` for `1/(2+cos(x))`. When `F` is `c*x` plus a function of `tan(u)` with one period, every jump is the same and is computed once (`equalJumps`), since each costs two limits. It returns the call unevaluated rather than a doubtful number: more than 100 different jumps, a complex log in `F` whose argument crosses the negative real axis (`crossesBranchCut`), logs with other symbols at a singular point. Constant factors are taken out of the integrand first. Symbolic bounds or a symbolic shift (`cos(x+y)`) are not analysed: the result is the generic formula.
...
### Certified sign of constants

No decision about a symbol-free constant is read off a double: `sin(3^34)`, `exp(pi*sqrt(163))-262537412640768744` or `sqrt(10^20+1)-10^10` have the wrong sign or are `0.0` there. `constantSign(p)` in [sources/assume.ts](sources/assume.ts) asks `certifiedSign` of [sources/bigfloat.ts](sources/bigfloat.ts) (two fixed-point evaluations 25 digits apart that must agree to 12 digits, at 30, 60, then 120 digits; size limits so that it cannot hang) and answers `1`, `-1`, `0` (cannot be told from zero: an exact zero in disguise) or `undefined` (no real evaluation: unsupported function such as `floor`, complex intermediate value); it is cached per cons cell. `facts()` uses it for constants its rules know to be real, `compare()` in `sources/test.ts` (comparisons, `and`/`or` bounds, `min`/`max`, `median`) for the difference of two constants, where "equal" additionally needs a proof (`simplify` of the difference gives 0), and `isZeroLikeOrNonZeroLikeOrUndetermined` for the truth of a constant; that one also leaves an undecided comparison or `and`/`or`/`not` alone instead of evaluating it again in floats (float coefficients made `simplify`/`gcd` stop or recurse without end). Undecided means unevaluated, as for a symbolic condition. `float(x)` sends `sin`, `cos`, `tan` of an exact argument beyond 2^50 through `bigFloat`.
...
### Series: convergence and poles

An infinite `sum` judges divergence on the whole summand, never on one of its parts (`infiniteSum` in [sources/sum.ts](sources/sum.ts)): the polynomial and rational terms form one part that converges exactly when the denominator degree exceeds the numerator degree by 2; a helper returns `DIVERGES` instead of stopping, and the Stop is only raised when every other part has a closed form. Partial fractions that do not telescope are summed as `-sum c_i*digamma(a+m_i)`, exactly for denominators 1, 2, 3, 4, 6 (Euler's constant drops out), else with symbolic `digamma`. Before any closed form, `hasIntegerPole` stops with `divide by zero` when the summand has a pole at an integer from a numeric lower limit on. `dsolve` numbers its constants around the names `C1, C2, ...` that occur in the input.
