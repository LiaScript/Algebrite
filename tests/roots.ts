...
run_test([
  ...
  'roots(thePoly)',
  // three real roots: the trigonometric form, ascending (see roots_trig.ts)
...
run_test([
  ...
  // x = t-1 gives t^3-12*t+8, and t = 4*c gives 8*(8*c^3-6*c+1), which is
  // cos(3*u) = -1/2 for c = cos(u)
  '[-1+4*cos(8/9*pi),-1+4*cos(4/9*pi),-1+4*cos(2/9*pi)]',
